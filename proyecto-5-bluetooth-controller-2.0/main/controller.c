...
static void button_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        // Preparar botones del gamepad (16 bits: Button 1-16)
        // Mapeo físico a lógico según estándar HID Gamepad:
        // - Button 1 (bit 0 = 0x0001): Botón A físico (GPIO 13) - Acción principal
        // - Button 2 (bit 1 = 0x0002): Botón B físico (GPIO 12) - Acción secundaria
        // - Button 3-10 (bits 2-9): Reservados para futuros botones (X, Y, L1, R1, L2, R2, etc.)
        // - Button 11 (bit 10 = 0x0400): Botón SELECT físico (GPIO 25)
        // - Button 12 (bit 11 = 0x0800): Botón START físico (GPIO 26)
        // - Button 13-16 (bits 12-15): Reservados para futuros botones
        //
        // NOTA: Aumentamos el rango a Button 16 para probar si Button 11 y 12 se reconocen mejor.
        uint16_t gamepad_buttons = 0;
        
        // Mapear botones directamente - asegurar que los bits se establezcan correctamente
        if (a_pressed) {
            gamepad_buttons |= 0x0001;       // Button 1 -> A físico (GPIO 13) - bit 0
        }
        if (b_pressed) {
            gamepad_buttons |= 0x0002;       // Button 2 -> B físico (GPIO 12) - bit 1
        }
        if (select_pressed) {
            gamepad_buttons |= 0x0400;       // Button 11 -> SELECT físico (GPIO 25) - bit 10
        }
        if (start_pressed) {
            gamepad_buttons |= 0x0800;       // Button 12 -> START físico (GPIO 26) - bit 11
        }
        ...
        if (select_pressed || start_pressed) {
            ...
            ESP_LOGI(TAG, ">>> gamepad_buttons word: 0x%04X (binario: 0b%016b)", gamepad_buttons, gamepad_buttons);
            ESP_LOGI(TAG, ">>> Bits individuales - A: %d, B: %d, SEL: %d (bit10/Button11), ST: %d (bit11/Button12)",
                     (gamepad_buttons & 0x0001) ? 1 : 0,
                     (gamepad_buttons & 0x0002) ? 1 : 0,
                     (gamepad_buttons & 0x0400) ? 1 : 0,
                     (gamepad_buttons & 0x0800) ? 1 : 0);
        }
        ...
        static uint16_t last_gamepad_buttons = 0;
        ...
        if (current_connected) {
            ...
            if (should_send) {
                ...
                if (buttons_changed) {
                    ESP_LOGI(TAG, "Gamepad ENVIADO - Buttons: 0x%04X (A:%d B:%d SEL:%d ST:%d), Hat: %d, X: %d, Y: %d", 
                             gamepad_buttons, 
                             (gamepad_buttons & 0x0001) ? 1 : 0,
                             (gamepad_buttons & 0x0002) ? 1 : 0,
                             (gamepad_buttons & 0x0400) ? 1 : 0,  // Button 11 (bit 10)
                             (gamepad_buttons & 0x0800) ? 1 : 0,  // Button 12 (bit 11)
